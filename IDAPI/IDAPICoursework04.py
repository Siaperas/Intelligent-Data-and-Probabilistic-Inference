...
def Covariance(theData):
    ...
    covar= dot(U.T,U) / (len(realData) - 1)
    # Coursework 4 task 2 ends here
    return covar

def CreateEigenfaceFiles(theBasis,name):
    # Coursework 4 task 3 begins here
    for i in xrange(len(theBasis)):
      pc = name + str(i+1) + ".jpg"
      SaveEigenface(theBasis[i], pc)
    # Coursework 4 task 3 ends here

def ProjectFace(theBasis, theMean, theFaceImage):
    magnitudes = []
    # Coursework 4 task 4 begins here
    data=ReadOneImage(theFaceImage)
    magnitudes= dot((data-theMean),theBasis.T)
    # Coursework 4 task 4 ends here
    return array(magnitudes)

def CreatePartialReconstructions(aBasis, aMean, componentMags,name):
    # Coursework 4 task 5 begins here
    SaveEigenface(aMean, name + ".jpg")
    for i in xrange(len(componentMags)):
      reconstruction = add(dot(aBasis[0:i].T, componentMags[0:i]), aMean)
      SaveEigenface(reconstruction, name +str(i+1)+".jpg")
...
def PrincipalComponents(theData):
    ...
    eigenvalues , eigenvectors = linalg.eig(dot(U, U.T))
    phi = dot(U.T, eigenvectors).T
    
    for i in xrange(len(phi)):
        magnitude = sqrt(dot(phi[i], phi[i].T))
        phi[i] /= magnitude
    
    inds = eigenvalues.argsort()[::-1][:len(eigenvalues)]
    orthoPhi = phi[inds]
...
noVariables, noRoots, noStates, noDataPoints, datain = ReadFile("HepatitisC.txt")
theData = array(datain)
AppendString("IDAPIResults04.txt","Coursework Four Results by Pavlos Siaperas")
mean=Mean(theData)
covariance=Covariance(theData)
AppendString("IDAPIResults04.txt","The mean vector Hepatitis C data set")
AppendList("IDAPIResults04.txt", mean)
AppendString("IDAPIResults04.txt","The covariance matrix of the Hepatitis C data set")
AppendArray("IDAPIResults04.txt", covariance)

face="c.pgm"

basis = ReadEigenfaceBasis()
CreateEigenfaceFiles(basis,"EigenFace")
meanimage = array(ReadOneImage("MeanImage.jpg"))
magnitudes = ProjectFace(basis, meanimage, face)
AppendString("IDAPIResults04.txt","The component magnitudes for image “c.pgm” in the principal component basis used in task 4.4")
AppendList("IDAPIResults04.txt", magnitudes)
CreatePartialReconstructions(basis, meanimage, magnitudes,"Reconstrutcion")

image=array(ReadImages())
newmeanimage=Mean(image)
newbasis = PrincipalComponents(image)
CreateEigenfaceFiles(newbasis,"NewEigenFace")
newmagnitudes = ProjectFace(newbasis, newmeanimage, face)
CreatePartialReconstructions(newbasis, newmeanimage, newmagnitudes,"NewReconstrutcion")
AppendString("IDAPIResults04.txt","The component magnitudes for image “c.pgm” in the principal component basis used in task 4.4")
AppendList("IDAPIResults04.txt", newmagnitudes)
